...
# ╔═╡ ddf2a524-534b-45f5-bae4-21e66444dbd9
Ryy(theta) = RotationGate(kron(Y, Y), theta)

# ╔═╡ adf8676e-df45-4ae2-949d-95a460711627
Ryz(theta) = RotationGate(kron(Y, Z), theta)

# ╔═╡ 3330a362-3eba-4188-85d7-10a7ffb7314d
Rzy(theta) = RotationGate(kron(Z, Y), theta)

# ╔═╡ 1897ac2e-1c34-4d42-b6f9-951d9020bbf5
begin
	Rxz(theta) = RotationGate(kron(X, Z), theta)
	Rzx(theta) = RotationGate(kron(Z, X), theta)
	Rxy(theta) = RotationGate(kron(X, Y), theta)
	Ryx(theta) = RotationGate(kron(Y, X), theta)
end

# ╔═╡ a81ef31d-c588-4b73-809b-c98171874130
one_site_hopping(tau, i) = chain( 
	put((i,i+1)=>Rxx(tau)),
	put((i,i+1)=>Ryy(tau)),
	put((i,i+1)=>Rzz(tau)),
	put((i,i+1)=>Ryz(tau)),
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "c75538abb40fe760365784d01a284c4440d96dfe"
